...
@pytest.mark.parametrize("user_query, expected_data", [
...
def test_parse_user_request(llm_request_parser, user_query, expected_data):
    ...
    assert \
        (llm_request_parser.request_context.date == expected_data)\
        or\
        (expected_data != "" and expected_data in llm_request_parser.request_context.date)
